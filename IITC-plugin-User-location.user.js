...
function wrapper(plugin_info) {
    ...
    window.plugin.userLocation.success = function(pos) {
        ...
        var hSize=64;
        var wSize=64;
        var usrLatLong ={"lat":crd.latitude,"lng":crd.longitude}
        ...
        if(crd.latitude < map.getBounds().getSouthWest().lat||
           ...
           crd.longitude >  map.getBounds().getNorthEast().lng){
            console.log('Lat: '+crd.latitude +' ::'+ map.getBounds().getSouthWest().lat +', ' + map.getBounds().getNorthEast().lat+ ' Lon: '+crd.longitude +'::'+ map.getBounds().getSouthWest().lng +', '+  map.getBounds().getNorthEast().lng  );
            map.setView(usrLatLong);
        }
        var lLine = null;
        var latLngs;
        var ll1,ll2
